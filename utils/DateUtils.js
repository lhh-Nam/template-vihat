...
import { isUndefined, isLeapYear, isNumber, isNumeric } from './Utils';
...
export function dateDiff(type, past, present) {
    ...
    switch (type) {
        case 'ms':
            return parseInt((t2 - t1).toString());
        case 's':
            return parseInt(((t2 - t1) / (1000)).toString());
        case 'min':
            return parseInt(((t2 - t1) / (60 * 1000)).toString());
        case 'h':
...
export function getTotalDayOfYear(date) {
    if (!date) { return null; }
    let year;
    if (date.toString().length !== 4 && isDate(date)) {
