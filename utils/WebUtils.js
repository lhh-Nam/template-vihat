...
import { defaultSubDomain } from '../constants/Domain';
...
export function redirectToLoginPage({ reload, clearCookie } = {}) {
    ...
    let targetSubdomain = defaultSubDomain, baseAddress = `.${getDomainWeb()}`;
    if (port) { baseAddress += `:${port}` }
    if (clearCookie) { setCookie({ sourceVersion }); }
    window.location.href = `${protocol}//${targetSubdomain}${baseAddress}/`;
